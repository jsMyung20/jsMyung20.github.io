...
function render() {
    ...
    // Clear canvas
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);  // DEPTH_BUFFER_BIT 추가
    gl.enable(gl.DEPTH_TEST);

    // Viewing transformation matrix
    let camX = cameraCircleRadius * Math.sin(glMatrix.toRadian(cameraCircleSpeed * elapsedTime));  // camX = rsin(theta)
    let camZ = cameraCircleRadius * Math.cos(glMatrix.toRadian(cameraCircleSpeed * elapsedTime));  // camZ = rcos(theta)
    let camY = 5 + 5 * Math.sin(glMatrix.toRadian(cameraHeightSpeed * elapsedTime));
    mat4.lookAt(viewMatrix, 
        vec3.fromValues(camX, camY, camZ), // camera position
        vec3.fromValues(0, 0, 0), // look at origin
        vec3.fromValues(0, 1, 0)); // up vector (approximate)
