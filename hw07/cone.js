export class Cone {
    ...
    constructor(gl, segments = 32, options = {}) {
        ...
        for (let i = 0; i < segments; i++) {
            ...
            const vecA = vec3.sub(vec3.create(), [x0_bot, -0.5, z0_bot], top);
            const vecB = vec3.sub(vec3.create(), [x1_bot, -0.5, z1_bot], top);
            const normal = vec3.cross(vec3.create(), vecB, vecA);
            const size = Math.sqrt(normal[0]*normal[0] + normal[1]*normal[1] + normal[2]*normal[2]);

            const nx = normal[0] / size;
            const ny = normal[1] / size;
            const nz = normal[2] / size;
